...
public class ComponentRuntimeFactory implements IComponentRuntimeFactory {
    ...
    synchronized public IComponentRuntime create(String id, ComponentContext context, int threadNumber) {
        try {
            XMLComponent definition = componentDefinitionFactory.getDefinition(id);
            if (definition != null) {
                IComponentRuntime component = (IComponentRuntime) Class.forName(definition.getClassName().trim()).newInstance();
