...
public class DesignNavigator extends AbstractFolderNavigator implements IDesignNavigator {
	...
	@Override
	protected void addMenuButtons(MenuItem newMenu, MenuBar leftMenuBar,
		...
		newComponent = newMenu.addItem("Component", Icons.COMPONENT,
				new Command() {
		...
		newResource.addItem("Local File System", Icons.FILE_SYSTEM,
				new Command() {
	...
	protected void openItem(Object item) {
		if (item instanceof FlowStep) {
			...
			if (flowStep.getComponent().getType().equals(FixedLengthFormatter.TYPE)) {
				EditFormatPanel editFormat = new EditFormatPanel(context,
						flowStep.getComponent());
				tabs.addCloseableTab(flowStep.getId(), flowStep.getName(), Icons.COMPONENT, editFormat);
			} else {
				item = configurationService.findFlow(flowStep.getFlowId());
			}
		}

		if (item instanceof Flow) {
			Flow flow = (Flow) item;
			EditFlowPanel flowLayout = new EditFlowPanel(context, flow, this,
					tabs);
			...
			tabs.addCloseableTab(model.getId(), model.getName(), Icons.MODEL,
					editModel);
			...
			tabs.addCloseableTab(resource.getId(), resource.getName(),
					treeTable.getItemIcon(item), sheet);
		}
	}
	...
	protected void selectionChanged(ValueChangeEvent event) {
		...
		boolean enabled = getSelectedFolder() != null
		...
		if (treeItems.size() > 0) {
			...
			if (object instanceof Flow) {
				...
				ConfirmDialog.show("Delete Flow?",
						"Are you sure you want to delete the " + flow.getName()
								+ " flow?", new DeleteFlowConfirmationListener(
								flow, treeItems));
	...
	protected void addResourcesToFolder(Folder folder) {
		List<Resource> resources = configurationService
				.findResourcesInFolder(folder);
