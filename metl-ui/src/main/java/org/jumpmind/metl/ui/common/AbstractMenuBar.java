...
abstract public class AbstractMenuBar extends MenuBar {
    ...
    private void setContextMenuEnabled(ISelectedValueMenuManager action, List<MenuItem> items, Object selected) {
        if (items != null) {
            for (MenuItem menuItem : items) {
                menuItem.setEnabled(action.isEnabled(buildMenuString(menuItem), selected));
                List<MenuItem> children = menuItem.getChildren();
    ...
    private MenuItem addToContextMenuIfNotExists(String name,
            MenuItem parent,
            List<MenuItem> items) {
        MenuItem item = null;
        if (items != null) {
