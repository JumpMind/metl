...
public class LdapPanel extends Panel implements IUiPanel {
	...
	public LdapPanel(final ApplicationContext context, TabbedPanel tabbedPanel) {
		...
		final GlobalSetting securityPrincipalSetting = getGlobalSetting(GlobalSetting.LDAP_SECURITY_PRINCIPAL,
				GlobalSetting.LDAP_SECURITY_PRINCIPAL_DEFAULT);

		FormLayout form = new FormLayout();
		form.setSpacing(true);

		hostField = new ImmediateUpdateTextField(MessageSource.message("ldapPanel.LDAP.URL")) {
			private static final long serialVersionUID = 1L;
		...
		baseDnField = new ImmediateUpdateTextField(MessageSource.message("ldapPanel.Base.DN")) {
			private static final long serialVersionUID = 1L;
	...
	class TestClickListener implements ClickListener {
		...
		public void buttonClick(ClickEvent event) {
			UI.getCurrent().addWindow(new LdapTestDialog());
		}
	}
	...
	class LdapTestDialog extends ResizableWindow {
		...
		protected void addButtonFooter() {
			...
			Button cancelButton = new Button(MessageSource.message("ldapPanel.done"));
		...
		private void testAuthentication() {
			try {
				 ...
				 * user/password combo. Currently if the host is wrong, it infinite loops
